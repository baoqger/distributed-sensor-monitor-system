...
func main() {
	...
	defer conn.Close()
	defer ch.Close()
	// Each sensor creates a new queue bind to the default direct exchange
	// And the queue's name is sensor name
	dataQueue := qutils.GetQueue(*name, ch)

	// Each sensor publishes online information(sensor name) to the fanout exchange
	...
	for range signal {
		...
		// Each sensor publishes measurement value to the default direct exchange
		// Publish with routing key as sensorName
		ch.Publish(
			"",             //exchange string,
